 ...
 * Run with: node e2e-record.mjs  (dev server must be running)
...
/**
 * Hover → brief pause → click.
 * The hover triggers mousemove so the cursor ring moves to the element before clicking.
 */
async function hoverClick(page, selector, wait = 400) {
...
/**
 * Injects an indigo cursor ring that tracks mouse and pulses on clicks.
 * addInitScript survives page.goto() navigations automatically.
 */
async function injectCursorHighlight(page) {
  await page.addInitScript(() => {
    ...
    Object.assign(ring.style, {
      ...
      pointerEvents: 'none',
      zIndex: '999999',
      left: '-100px', // off-screen until first mousemove
      top: '-100px',
    ...
    document.addEventListener('mousedown', () => {
      ...
      ring.style.backgroundColor = 'rgba(99,102,241,0.35)'
    })
...
try {
  ...
  const browser = await chromium.launch({
    headless: false,
    slowMo: 800, // ms between each Playwright action — makes interactions deliberate
  ...
  await page.waitForTimeout(2000) // let viewer read the landing
  log('landing loaded')
  ...
  await page.hover('input[placeholder*="Viagem"]')
  await page.waitForTimeout(300)
  await page.fill('input[placeholder*="Viagem"]', 'Viagem PG · Abril 2026')
  await page.waitForTimeout(600)

  await page.hover('input[placeholder="Nome da pessoa"]')
  await page.waitForTimeout(300)
  await page.fill('input[placeholder="Nome da pessoa"]', 'Rafa')
  await hoverClick(page, 'button:has-text("+")')
  ...
  const [apiResponse] = await Promise.all([
    ...
    hoverClick(page, 'button:has-text("CRIAR ACERTO")', 600),
  ])
  ...
  await hoverClick(page, 'button:has-text("CHAT")')
  await page.waitForTimeout(800)
  ...
  await hoverClick(page, 'button:has-text("EXTRAIR TRANSAÇÕES")', 600)
  log('extracting (Gemini)...')
  await page.waitForSelector('text=R$', { timeout: 60000 })
  ...
  await hoverClick(page, 'button:has-text("Buckets")')
  await page.waitForTimeout(1000)

  await hoverClick(page, 'button:has-text("Transações")')
  await page.waitForTimeout(800)

  await hoverClick(page, 'a:has-text("VER QUEM DEVE QUANTO")', 600)
  ...
  // Scroll down to reveal PIX transfers
  ...
  // Extract at 15fps; gifski will play back at 10fps → 1.5× slower = more readable
  execSync(
    `ffmpeg -y -i '${videoPath}' -vf "fps=15,scale=960:-1:flags=lanczos,fade=in:st=0:d=0.5" '${FRAMES_DIR}/frame%04d.png'`,
    { stdio: 'pipe' }
  )
  ...
  step('Encoding GIF with gifski (10fps playback)')
  mkdirSync('docs', { recursive: true })

  execSync(
    `gifski --fps 10 --quality 92 -o '${OUT_GIF}' '${FRAMES_DIR}/frame'*.png`,
    { stdio: 'inherit' }
  )
